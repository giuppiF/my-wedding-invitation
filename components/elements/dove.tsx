...
export const Dove = ({ guest }: { guest: Guest }) => {
  return (
    <div className="flex flex-col gap-8">
      <p>Nelle ridenti colline torinesi, vicino a Superga per capirci.</p>
      {!guest.church && (
        <p>
          Il ritrovo è per una grande festa tutti insieme, che avrà luogo dopo
          una piccola cerimonia a causa delle dimensioni ristrette della
          chiesetta.{" "}
        </p>
      )}
      {guest.church && (
      ...
      <p className=" ">
        {guest.church && <span className="font-bold">Rinfresco:</span>} Centro
        Paluc, Baldissero T.se (TO){" - "}
        <a
